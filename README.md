...
- **Body Only (Pretty JSON)** — a read-only response-viewer tab (Proxy / Repeater / anywhere) that shows *just* the body with JSON XSSI guards (`)]}'`, `for(;;);`, `while(1);`) stripped and the JSON pretty-printed by a dependency-free re-indenter; the same body-only pretty JSON is an opt-in when you **Save request(s) for AI**.
...
- **Show only unique requests** — opens a separate window styled like Burp's HTTP history: columns for `# / Host / Method / URL / Status / Length / MIME / Notes`, rows tinted by their Burp **highlight color** (so attacker=green / victim=red / other=yellow carry over), the **Notes** column showing the `[DEDUPE] …` verdict + `[attacker]/[victim] port N` tag, and read-only request/response viewers beneath. Burp's API can't filter its own history table, so the deduplicated set is shown here instead. (Out-of-scope/static `SKIP` rows and known duplicates are excluded.)
  ...
  - **Save request(s) for AI** — saves all selected requests **and their responses into one `.http` file** (each in its own `####` section, prefixed with its [case manifest](#case-manifest-per-request)) for Claude Code / AI to read; pick the destination in the save dialog. Tick **Responses: body only, pretty JSON** in the dialog to write each response as just its body (XSSI guards stripped, JSON pretty-printed) instead of the full raw response — the choice is remembered.
